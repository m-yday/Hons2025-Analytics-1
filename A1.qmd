...
#| echo: false
#| warning: true # change this later!
#| output: false
#| message: true # also change this later!
...
lm_full <- lm(Revenue ~ ., train_data, family=binomial)
coef(lm_full) %>% # tidy() %>%
...
elasticnet <- cva.glmnet(Revenue ~., train_data, alpha = seq(0, 1, 0.1), nfolds=10, family=binomial)
plot(elasticnet)
