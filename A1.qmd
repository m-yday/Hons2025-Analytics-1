...
```{r}
train_stand_class <- train_stand |>
  mutate(Revenue = as.factor(Revenue))

shopping_tree_gini <- tree(Revenue ~ ., data = train_stand_class, split = 'gini')
plot(shopping_tree_gini)
text(shopping_tree_gini, pretty = 0)

# First, grow a slightly larger tree
shopping_bigtree <- tree(Revenue ~ ., data = train_stand_class, control = tree.control(nobs = nrow(na.omit(train_stand_class)), mindev = 0.005))
...
Looking at this plot, we appear to need no more than 4 terminal nodes. We plot the pruned tree corresponding to 4 nodes, so that we do not lose any categorising power.

```{r}
#| label: "Prune the tree"

shopping_pruned <- prune.tree(shopping_bigtree, best=4)
plot(shopping_pruned)
text(shopping_pruned, pretty = 0)
```

The split on the left based on Month predicts the same class for both sides of the split. This is strange, so we investigate the deviance and homogeneity at each of these nodes to see what is happening.
