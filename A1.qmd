...
shopping_bigtree <- tree(Revenue ~ ., data = train_class, control = tree.control(nobs = nrow(na.omit(train_stand_class)), mindev = 0.005))
...
shopping_pruned <- prune.tree(shopping_bigtree, best=3)
...
To decide on a tree size, we will also need the confusion matrix.
