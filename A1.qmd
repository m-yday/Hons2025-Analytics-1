...
A few of the splits predict the same class for both sides of the split. This is strange, so we investigate the deviance and homogeneity at each of these nodes to see what is happening. To decide on a tree size, we will also need the confusion matrix.
